...
z2 = Theta1 * a1'; % (25 X 401) X (401 X 5000) = 25 X 5000
a2 = sigmoid(z2); % 25 X 5000

a2 = a2'; % 5000 X 25
a2 = [ones(m, 1) a2]; % 5000 X 26   %Add Bias
...
I = eye(num_labels); % K X K (10 X 10)
y_matrix = I(y, :); % 5000 X 10 (m X K)
...
regulariaztion = (lambda / (2 * m)) * ( sum(sum(Theta1(:, 2 : end) .^ 2)) + sum(sum(Theta2(:, 2 : end) .^ 2)) );
J = J + regulariaztion;

% Backpropagation

d3 = a3 - y_matrix; % 5000 X 10
d2 = (d3 * Theta2) .* [ones(size(z2'), 1) sigmoidGradient(z2')]; % (5000 X 10) X (10 X 26) = 5000 X 26
